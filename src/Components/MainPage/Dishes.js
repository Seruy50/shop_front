...
const Dish = ({ name, price, id, shopId, cartItemShopid }) => {
  const addToCart = () => {
    ...
    if (!data.find((d) => d._id === id)) {
      arr = [
        ...data,
        { count: 1, name: name, price: price, _id: id, shopId: shopId },
      ];
    } else {
      return;
    }
  ...
  return (
    <div className="dishes__card">
      ...
      <div>
        <button
          className="dishes__card__button"
...
export default function Dishes({ shopId }) {
  ...
  useEffect(() => {
    axios
      .get(`https://foodshop-back.onrender.com/shops/${shopId}`)
      .then((data) => {
        setDishes(data.data);
      })
